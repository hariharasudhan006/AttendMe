...
import com.android.attendme.ml.mTCNN.Box;
...
public class MTCNN {
    ...
    private final Interpreter pInterpreter;
    private final Interpreter rInterpreter;
    private final Interpreter oInterpreter;
        ...
        for (int y = 0; y < h; y++) {
            for (int x = 0; x < w; x++) {
                ...
                if (score > pNetThreshold) {
                    ...
                    System.arraycopy(conv4_2_BiasAdd[0][y][x], 0, box.bbr, 0, 4);
    ...
    private void rNetForward(float[][][][] rNetIn, Vector<Box> boxes) {
        ...
        for (int i = 0; i < num; i++) {
            ...
            System.arraycopy(conv5_2_conv5_2[i], 0, boxes.get(i).bbr, 0, 4);
        }
    }
    ...
    private void oNetForward(float[][][][] oNetIn, Vector<Box> boxes) {
        ...
        for (int i = 0; i < num; i++) {
            boxes.get(i).score = prob1[i][1];
            System.arraycopy(conv6_2_conv6_2[i], 0, boxes.get(i).bbr, 0, 4);
            for (int j = 0; j < 5; j++) {
